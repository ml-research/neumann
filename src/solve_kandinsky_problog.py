...
from neumann_utils import (get_clause_evaluator, get_data_loader, get_model,
                           get_prob)
from problog_utils import to_ProbLog_program_batch
   ...
   #print(ctl.solve(on_model=print))
    ...
    for epoch in tqdm(range(args.epochs)):
        for i, sample in enumerate(tqdm(loader), start=0):
            ...
            #NEUMANN.print_valuation(V_T)
            problog_program_batch= to_ProbLog_program_batch(V_0, NEUMANN)
            for problog_program in tqdm(problog_program_batch):
                get_evaluatable().create_from(problog_program).evaluate()


            # clingo_texts = NEUMANN.to_clingo_text_batch(V_0)
            # for clingo_text in clingo_texts:
            #     ctl = Control()
            #     ctl.add(clingo_text)
            #     ctl.ground()
            #     ctl.solve()
...
def main(n):
    ...
    for j in range(n):
        acc_test, rec_test, th_test, time = predict(
            NEUMANN, I2F, train_loader, args, device, th=0.5, split='train')
        times.append(time)
    
    with open('out/inference_time/time_problog_{}_ratio_{}.txt'.format(args.dataset, args.n_ratio), 'w') as f:
