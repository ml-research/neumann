...
def eval_clauses(args, NEUMANN, I2F, optimizer, train_loader, val_loader, test_loader, device, writer, rtpt, trial):
    ...
    clause_scores = NEUMANN.clause_weights.squeeze(0).detach()
...
def main(n):
    ...
    NEUMANN, I2F = get_model(lang=lang, clauses=clauses, atoms=atoms, terms=terms, bk=bk, bk_clauses=bk_clauses,
                             ...
                             num_objects=args.num_objects, infer_step=args.infer_step, train=True) #train=False) #train=not(args.no_train))
    # clause generator
    ...
    for j in range(n):
        ...
        while trial < args.trial:
            ...
            pos_ratio = 0.2
            neg_ratio = pos_ratio * 0.5 #pos_ratio * 0.1
            ...
            optimizer.zero_grad()
            clause_scores = eval_clauses(args, NEUMANN, I2F, optimizer, train_loader, val_loader, test_loader, device, writer, rtpt, trial=trial)
            NEUMANN.print_program()
            trial += 1
        ...
        train_loader, val_loader, test_loader = get_data_loader(args, device, pos_ratio=1.0, neg_ratio=1.0)
        ...
        with open('out/learning_time/time_neumann_{}_ratio_{}.txt'.format(args.dataset, args.n_ratio), 'w') as f:
            f.write("\n".join(str(item) for item in times))
        with open('out/learning_time/validation_accuracy_neumann_{}_ratio_{}.txt'.format(args.dataset, args.n_ratio), 'w') as f:
            f.write("\n".join(str(item) for item in val_accs))
        with open('out/learning_time/test_accuracy_neumann_{}_ratio_{}.txt'.format(args.dataset, args.n_ratio), 'w') as f:
