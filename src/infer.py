...
class InferModule(nn.Module):
    ...
    def __init__(self, I, m, infer_step, gamma=0.01, device=None, train=False):
        ...
        if not train:
            ...
            #    np.random.normal(size=(m, I.size(0)))).to(device))a
            np.random.seed(0)
            self.W = nn.Parameter(torch.Tensor(
                np.random.rand(m, I.size(0))).to(device))
        # clause function
    ...
    def init_identity_weights(self, device):
        ones = torch.ones((self.C, ), dtype=torch.float32)
        return torch.diag(ones).to(device)
    
    def get_weights(self):
        return np.round(torch.softmax(self.W, 1).detach().cpu().numpy(), 2)
    ...
    def forward(self, x):
        ...
        """
        R = x
        self.V_list.append(R.detach().cpu().numpy()[:,2:])
