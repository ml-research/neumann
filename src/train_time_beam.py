...
def predict_no_return(NEUMANN, I2F, loader, args, device,  th=None, split='train'):
    predicted_list = []
    target_list = []
    count = 0
...
def main(n):
    ...
    wandb.init(project="NEUMANN", name="{}_time_beam:seed_{}".format(args.dataset, args.seed))
    ...
    while trial < args.trial:
        ...
        train_pos_loader, val_pos_loader,  test_pos_loader = get_data_loader(args, device, pos_ratio=1.0, neg_ratio=0.0)
        NEUMANN, new_gen_clauses = update_by_refinement(NEUMANN, clause_scores, clause_generator, softmax_temp=softmax_temp)
        print("new_gen_clauses", new_gen_clauses)
        start = time.time()
        for c in new_gen_clauses:
            print('Building NEUMANN to evaluate clause: ', c)
            NEUMANN_dummy = update_by_clauses(NEUMANN, [c])
            print("evaluating clauses using positive examples... ")
            predict_no_return(
                NEUMANN_dummy, I2F, val_pos_loader, args, device, th=0.5, split='train')
    ...
    with open('out/learning_time_neumann_beam_{}_{}.txt'.format(args.dataset, args.seed), 'w') as f:
