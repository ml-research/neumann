...
def train_neumann(args, NEUMANN, I2F, optimizer, train_loader, val_loader, test_loader, device, writer, rtpt, epochs, trial):
    ...
    for epoch in range(epochs):
        ...
        wandb.log({'metric_trial/training_loss_trial_{}'.format(trial): loss_i})
        ...
        """
    NEUMANN.print_program()
    # return loss.item(), NEUMANN.clause_weights.detach() #clause_scores / epochs
    beta = 10000
    return loss.item(), beta * clause_scores #/ epochs
...
def main(n):
    ...
    NEUMANN_init = NEUMANN
    clause_scores_init = clause_scores
    clause_generator_init = clause_generator
    ...
    while trial < args.trial:
        # if trial == args.trial:
        #     print('NEW TRIAL:')
        #     # initialize learning setting, keeping the refinement history
        #     NEUMANN = NEUMANN_init
        #     clause_scores = clause_scores_init
        #     history = clause_generator.refinement_history.difference(set(clauses))
        #     clause_generator = clause_generator_init
        #     clause_generator.refinement_history = history
        #     trial = 0
        ...
        optimizer.zero_grad()
        loss, clause_scores = train_neumann(args, NEUMANN, I2F, optimizer, train_loader, val_loader, test_loader, device, writer, rtpt, epochs=args.trial_epochs, trial=trial)
        
        # # sample good rules on positive examples (complete rules)
        # NEUMANN_, new_gen_clauses = update_by_refinement(NEUMANN, clause_scores, clause_generator, softmax_temp=softmax_temp, replace=True)
        # # TRAIN on NEGATIVE
        # # TODO
        # train_loader, val_loader,  test_loader = get_data_loader(args, device, pos_ratio=0.0, neg_ratio=0.2)
        # # clause_generator.print_tree()
        # params = list(NEUMANN_.parameters())
        # optimizer = torch.optim.RMSprop(params, lr=lr)
        # # optimizer = torch.optim.SGD(params, lr=lr)
        # optimizer.zero_grad()
        # loss_neg, clause_scores_neg = train_neumann(args, NEUMANN_, I2F, optimizer, train_loader, val_loader, test_loader, device, writer, rtpt, epochs=args.epochs, trial=trial)
        
        times.append(time.time() - start)

        print("Predicting on validation data set...")
        acc_val, rec_val, th_val = predict(NEUMANN, I2F, val_loader, args, device, th=0.5, split='val')
        wandb.log({'metric/accuracy_validation': acc_val})
        wandb.log({'metric_trial/accuracy_validation_trial_{}'.format(trial): acc_val})
        print("Predicting on test data set...")
        # test split
        acc_test, rec_test, th_test = predict(NEUMANN, I2F, test_loader, args, device, th=0.5, split='test')
        wandb.log({'metric/accuracy_test': acc_test})
        wandb.log({'metric_trial/accuracy_test_trial_{}'.format(trial): acc_test})

        # stop the trial when the loss is getting worse
        # if loss > 1.5 * loss_prev:
        #     # initialize learning setting, keeping the refinement history
        #     NEUMANN = NEUMANN_init
        #     clause_scores = clause_scores_init
        #     history = clause_generator.refinement_history.difference(set(clauses))
