...
class SlotAttentionMultiScenesPerceptionModule(nn.Module):
    ...
    def forward(self, imgs):
        # for vilp tasks, multiple scenes are given, without batch computation
        Z = self.model(imgs)
        # sort the output of slot attention w.r.t objectness and take top-e outputs
        sorted, indices = torch.sort(x[:,:,0], descending=True)
        return torch.gather(Z, 1, indices)[:,:self.e]


class SlotAttentionLessColorsPerceptionModule(nn.Module):
    """A perception module using Slot Attention.
    ...
    def to_less_colors(self, Z):
        # extract: cyan, gray, red, yellow
        print(Z.shape)
        # Z_color = torch.cat([Z[:,:,0], Z[:,:,6], Z[:,:,4], Z[:,:,2]], dim=2)
        # color_index = torch.tensor([10,16,14,12]).to(self.device)
        # color_index = torch.tensor([11,17,15,13]).to(self.device)
        color_index = torch.tensor([11,17,15,13]).to(self.device)
        # color_index = torch.tensor([17,11,13,15]).to(self.device)
        Z_color = Z[:,:,color_index]
        Z_other = Z[:,:,:11]
        return torch.cat([Z_other, Z_color], dim=2)

    def forward(self, imgs):
        Z = self.model(imgs)
        Z = self.to_less_colors(Z)
        # sort the output of slot attention w.r.t objectness and take top-e outputs
        sorted, indices = torch.sort(Z[:,:,0], descending=True)
        indices_e = indices[:, :self.e]
...
class SlotAttentionSlotsPerceptionModule(nn.Module):
    """A perception module using Slot Attention.
    ...
    def load_model(self):
        ...
        """
        if self.device == torch.device('cpu'):
            sa_net = SlotAttention_slots_model(n_slots=10, n_iters=3, n_attr=18,
                                         encoder_hidden_channels=64,
                                         attention_hidden_channels=128, device=self.device)
            log = torch.load(
                "src/weights/slot_attention/best.pt", map_location=torch.device(self.device))
