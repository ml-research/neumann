...
class SlotAttention_model(nn.Module):
    def __init__(self, n_slots, n_iters, n_attr,
        ...
        self.encoder_cnn = SlotAttention_encoder(
            in_channels=in_channels, hidden_channels=encoder_hidden_channels).to(device)
        self.encoder_pos = SoftPositionEmbed(
            encoder_hidden_channels, (32, 32), device=device)
        self.layer_norm = nn.LayerNorm(encoder_hidden_channels, eps=1e-05).to(device)
        self.mlp = MLP(hidden_channels=encoder_hidden_channels).to(device)
        self.slot_attention = SlotAttention(num_slots=n_slots, dim=encoder_hidden_channels, iters=n_iters, eps=1e-8,
                                            hidden_dim=attention_hidden_channels).to(device)
        self.mlp_classifier = SlotAttention_classifier(
            in_channels=encoder_hidden_channels, out_channels=self.n_attr).to(device)
...
class SlotAttention_slots_model(nn.Module):
    def __init__(self, n_slots, n_iters, n_attr,
                 in_channels=3,
                 encoder_hidden_channels=64,
                 attention_hidden_channels=128,
                 device="cuda"):
        super(SlotAttention_slots_model, self).__init__()
        self.n_slots = n_slots
        self.n_iters = n_iters
        self.n_attr = n_attr
        self.n_attr = n_attr + 1  # additional slot to indicate if it is a object or empty slot
